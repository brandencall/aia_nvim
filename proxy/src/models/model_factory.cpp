...
std::shared_ptr<models::BaseModel> createEndpointFromJson(const nlohmann::json &j) {
    ...
    if (id.find("Gemini-2.0-Flash") != std::string::npos) {
        return std::make_shared<models::GeminiTwoZeroFlash>(id, url, key, priority, rateLimitPerMinute,
                                                            rateLimitPerDay);
    } else if (id.find("Gemini-2.5-Flash") != std::string::npos) {
        return std::make_shared<models::GeminiTwoFiveFlash>(id, url, key, priority, rateLimitPerMinute,
                                                            rateLimitPerDay);
    }
